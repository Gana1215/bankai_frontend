// src/App.jsx — FINAL LOCKED (Stealth UI + Beautiful Toggle + Intent Buttons with display_mn)
// ✅ ONLY PATCH: TextChat intent list + click → sends intent key (clarify_click style)
...
function VoiceChat({ onBack }) {
    ...
    try {
      if (sttMode === "local") {
        if (!ready) {
          ...
          const t0 = Date.now();
          while (!ready && Date.now() - t0 < 12000) await new Promise((r) => setTimeout(r, 250));
          if (!ready) {
            showToast("⚠️ Модель ачаалагдсангүй");
            setLoading(false);
            return;
          }
        }

        showToast("🎧 Хөрвүүлж байна...");
        ...
        const backendText = data.text || data.transcript || data.asr_text || "";
        const inferMs = Math.round(performance.now() - t0);
        setLastTranscript(backendText);
        setLastPerf({ audioSec: null, inferMs, rtf: null, speedX: null });

        showToast("🤖 BankAI хариуллаа!");
      }
    } catch (err) {
      console.error(err);
      setError("⚠️ Холболтын алдаа гарлаа");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-blue-50 via-teal-50 to-white px-4 text-center relative">
      ...
      {(sttMode === "local" || (sttMode === "backend" && lastTranscript)) && (
...
// ====================================================
// 💬 TextChat — FINAL PATCH (display_mn + send intent key)
// ====================================================
function TextChat({ onBack }) {
  const [messages, setMessages] = useState([]);
  const [intents, setIntents] = useState([]); // [{name, display}]
  ...
  // 🔥 FINAL PATCH: backend provides {name, display} where display already prefers display_mn
  // But we still accept display_mn if backend adds it later.
  useEffect(() => {
    fetch(`${API_BASE}/intent/list_intents`)
      .then((r) => r.json())
      .then((d) => {
        if (!Array.isArray(d.intents)) return setIntents([]);

        setIntents(
          d.intents
            .filter((i) => i && (i.name || i.intent))
            .map((i) => {
              const name = i.name || i.intent;
              const display =
                i.display_mn ||
                i.display ||
                i.title ||
  ...
  // ✅ Click intent button → send intent key (no brain change)
  const sendIntentKey = async (intentKey, displayText) => {
    if (!intentKey) return;

    setMessages((m) => [...m, { role: "user", text: displayText || intentKey }]);
    setTyping(true);
    ...
    try {
      const res = await fetch(`${API_BASE}/intent/text_intent`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        // ✅ Uses existing backend direct-intent path (Phase2 untouched)
        body: JSON.stringify({ intent: intentKey, source: "clarify_click" }),
      });
  ...
  // Normal free text message (unchanged)
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-blue-50 via-teal-50 to-white text-center px-4 relative">
      ...
      {/* ✅ Intent buttons (display in MN, send intent key) */}
      {intents.length > 0 && (
        <div className="flex flex-wrap justify-center gap-3 mb-6 px-2 mt-4">
          {intents.map((it) => (
            <button
              key={it.name}
              onClick={() => sendIntentKey(it.name, it.display)}
              className="bg-gradient-to-r from-teal-500 to-blue-600 hover:from-blue-500 hover:to-teal-600 text-white font-semibold px-5 py-2 rounded-full shadow-md hover:shadow-lg transition-all duration-300"
              title={it.name}
            >
              {it.display}
            </button>
      ...
      <div className="flex-1 overflow-y-auto w-full max-w-md mx-auto bg-white rounded-2xl shadow-inner p-4 mb-4 mt-4">
        {messages.map((m, i) => (
          <div key={i} className={`mb-2 text-left ${m.role === "user" ? "text-blue-700" : "text-teal-700"}`}>
