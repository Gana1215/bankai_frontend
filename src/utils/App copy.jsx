...
function VoiceChat({ onBack }) {
  ...
  const [lastPerf, setLastPerf] = useState({ audioSec: null, inferMs: null, rtf: null, speedX: null });

  const initialMode =
    localStorage.getItem("sttMode") ||
    (TRANS_MODE_DEFAULT === "0" ? "local" : "backend");
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-blue-50 via-teal-50 to-white px-4 text-center relative">
      ...
      <div className="flex flex-col items-center gap-2 mb-4">
        <div className="text-[11px] font-mono uppercase tracking-widest text-gray-600">
          Model Mode
        </div>
      ...
      {(sttMode === "local" || (sttMode === "backend" && lastTranscript)) && (
        <div className="w-full max-w-md bg-white/70 border border-gray-200 rounded-2xl p-4 shadow-sm mb-6 text-left">
          <div className="flex items-center justify-between gap-3">
            <div className="text-[11px] font-mono uppercase tracking-widest text-gray-700">
              Transcript
            </div>
...
// ====================================================
// 💬 TextChat — PATCHED with display_mn
// ====================================================
function TextChat({ onBack }) {
  ...
  // 🔥 FINAL PATCH: load `display_mn` if exists
  useEffect(() => {
